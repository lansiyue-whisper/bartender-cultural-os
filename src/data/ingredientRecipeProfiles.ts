...
function buildProfile(seed: ProfileSeed): IngredientRecipeProfile {
  ...
  const collinsName = `${seed.ingredient} Collins`;
  const martiniName = `${seed.ingredient} Martini`;
  const oldFashionedName = `${seed.ingredient} Old Fashioned`;
  const fizzName = `${seed.ingredient} Fizz`;
  const punchName = `${seed.ingredient} Punch`;
  const margaritaName = `${seed.ingredient} Margarita`;
  const daiquiriName = `${seed.ingredient} Daiquiri`;
  const milkPunchName = `${seed.ingredient} Milk Punch`;
  const lowAbvName = `${seed.ingredient} Low ABV`;
  ...
  return {
    ...
    techniques: ['Cordial', 'Syrup', 'Infusion', 'Carbonation', 'Shrub', 'Clarification', 'Milk Punch'],
    pairings: seed.pairings,
    preparations: [
      seed.cordial,
      seed.syrup,
      `${seed.ingredient} Infusion`,
      `${seed.ingredient} Shrub`,
      `${seed.ingredient} Clarified Milk Punch`,
      `${seed.ingredient} Salt / Acid Adjust`,
    ],
    ...
    recipes: [
      ...
      {
        id: `${slug(seed.ingredient)}-collins`,
        ingredient: seed.ingredient,
        name: collinsName,
        style: 'Collins / 气泡长饮',
        confidence: 'Service-ready',
        bases: [seed.primarySpirit],
        ingredients: [seed.primarySpirit, seed.acid, seed.syrup, seed.bubbles, seed.garnish],
        flavorTags: ['长饮', '气泡', ...seed.tags].slice(0, 4),
        method: '摇合后补气泡 / Shake + Top',
        recipe: [
          { name: seed.primarySpirit, amount: '45 ml' },
          { name: seed.acid, amount: '20 ml' },
          { name: seed.syrup, amount: '15 ml' },
          { name: seed.bubbles, amount: '80-100 ml' },
          { name: seed.garnish, amount: '1 份' },
        ],
        steps: ['除气泡材料外加入摇壶。', '加冰短摇，让酸甜和香气打开。', '过滤入装冰高球杯。', '补气泡材料，轻轻提拉一次。'],
      },
      {
        id: `${slug(seed.ingredient)}-martini`,
        ingredient: seed.ingredient,
        name: martiniName,
        style: 'Martini / 清澈短饮',
        confidence: 'R&D starting point',
        bases: [seed.primarySpirit, 'Vermouth / Sake'],
        ingredients: [seed.primarySpirit, '干味美思 / 清酒', `${seed.ingredient} Infusion`, seed.garnish],
        flavorTags: ['清澈', '酒感', ...seed.tags].slice(0, 4),
        method: '搅拌 / Stir',
        recipe: [
          { name: seed.primarySpirit, amount: '50 ml' },
          { name: '干味美思 / 清酒', amount: '12.5 ml' },
          { name: `${seed.ingredient} Infusion`, amount: '5 ml' },
          { name: seed.garnish, amount: '1 份' },
        ],
        steps: ['所有液体加入装冰调酒杯。', '搅拌至充分冰镇并完成稀释。', '过滤入冰镇 Martini 杯。', '用装饰提供第一层香气。'],
      },
      {
        id: `${slug(seed.ingredient)}-old-fashioned`,
        ingredient: seed.ingredient,
        name: oldFashionedName,
        style: 'Old Fashioned / 酒感短饮',
        confidence: 'R&D starting point',
        bases: [seed.secondarySpirit],
        ingredients: [seed.secondarySpirit, seed.syrup, 'Aromatic Bitters', seed.garnish],
        flavorTags: ['酒感', '苦甜', ...seed.tags].slice(0, 4),
        method: '搅拌 / Stir',
        recipe: [
          { name: seed.secondarySpirit, amount: '50 ml' },
          { name: seed.syrup, amount: '7.5 ml' },
          { name: '芳香苦精', amount: '2 dash' },
          { name: seed.garnish, amount: '1 份' },
        ],
        steps: ['原料加入调酒杯。', '加冰搅拌至冰冷。', '过滤入装大冰块古典杯。', '用装饰完成香气，并控制甜度不要盖过基酒。'],
      },
      {
        id: `${slug(seed.ingredient)}-fizz`,
        ingredient: seed.ingredient,
        name: fizzName,
        style: 'Fizz / 轻盈气泡',
        confidence: 'Service-ready',
        bases: [seed.primarySpirit],
        ingredients: [seed.primarySpirit, seed.acid, seed.syrup, '蛋白 / Aquafaba', seed.bubbles],
        flavorTags: ['轻盈', '泡沫', ...seed.tags].slice(0, 4),
        method: '干摇后摇合 / Dry Shake + Shake',
        recipe: [
          { name: seed.primarySpirit, amount: '45 ml' },
          { name: seed.acid, amount: '20 ml' },
          { name: seed.syrup, amount: '15 ml' },
          { name: '蛋白 / Aquafaba', amount: '15 ml' },
          { name: seed.bubbles, amount: '60 ml' },
        ],
        steps: ['除气泡材料外先干摇。', '加冰再次摇合至泡沫细密。', '过滤入冰镇杯。', '补气泡材料，让香气更轻。'],
      },
      {
        id: `${slug(seed.ingredient)}-punch`,
        ingredient: seed.ingredient,
        name: punchName,
        style: 'Punch / 批量分享',
        confidence: 'R&D starting point',
        bases: [seed.primarySpirit, seed.secondarySpirit],
        ingredients: [seed.primarySpirit, seed.secondarySpirit, seed.cordial, seed.acid, seed.bubbles],
        flavorTags: ['批量', '分享', ...seed.tags].slice(0, 4),
        method: '预调 / Batch + Build',
        recipe: [
          { name: seed.primarySpirit, amount: '30 ml' },
          { name: seed.secondarySpirit, amount: '20 ml' },
          { name: seed.cordial, amount: '15 ml' },
          { name: seed.acid, amount: '15 ml' },
          { name: seed.bubbles, amount: '60-90 ml' },
        ],
        steps: ['提前混合除气泡外的原料并冷藏。', '出品时倒入装冰杯。', '补气泡材料。', '根据批量测试结果微调酸度和甜度。'],
      },
      {
        id: `${slug(seed.ingredient)}-margarita`,
        ingredient: seed.ingredient,
        name: margaritaName,
        style: 'Margarita / 龙舌兰酸酒',
        confidence: 'Service-ready',
        bases: ['Tequila / Mezcal'],
        ingredients: ['Tequila / Mezcal', seed.acid, seed.cordial, '盐水 / Saline', seed.garnish],
        flavorTags: ['酸咸', '龙舌兰', ...seed.tags].slice(0, 4),
        method: '摇合 / Shake',
        recipe: [
          { name: 'Tequila / Mezcal', amount: '45 ml' },
          { name: seed.acid, amount: '22.5 ml' },
          { name: seed.cordial, amount: '15 ml' },
          { name: '盐水 / Saline', amount: '2 dash' },
          { name: seed.garnish, amount: '1 份' },
        ],
        steps: ['所有液体加入摇壶。', '加冰充分摇合。', '双重过滤入冰镇杯或装冰杯。', '用盐水把酸度和香气边缘拉清楚。'],
      },
      {
        id: `${slug(seed.ingredient)}-daiquiri`,
        ingredient: seed.ingredient,
        name: daiquiriName,
        style: 'Daiquiri / 朗姆酸酒',
        confidence: 'Service-ready',
        bases: ['Rum / Cachaça'],
        ingredients: ['Rum / Cachaça', seed.acid, seed.syrup, seed.garnish],
        flavorTags: ['酸甜', '朗姆', ...seed.tags].slice(0, 4),
        method: '摇合 / Shake',
        recipe: [
          { name: 'Rum / Cachaça', amount: '50 ml' },
          { name: seed.acid, amount: '20 ml' },
          { name: seed.syrup, amount: '15 ml' },
          { name: seed.garnish, amount: '1 份' },
        ],
        steps: ['所有原料加入摇壶。', '加冰摇至杯壁结霜。', '过滤入冰镇 Coupe 杯。', '如果食材香气偏轻，增加 5 ml Cordial 替代部分糖浆。'],
      },
      {
        id: `${slug(seed.ingredient)}-milk-punch`,
        ingredient: seed.ingredient,
        name: milkPunchName,
        style: 'Milk Punch / 澄清奶洗',
        confidence: 'R&D starting point',
        bases: [seed.primarySpirit, seed.secondarySpirit],
        ingredients: [seed.primarySpirit, seed.secondarySpirit, seed.cordial, seed.acid, '牛奶 / Milk'],
        flavorTags: ['澄清', '丝滑', ...seed.tags].slice(0, 4),
        method: '奶洗澄清 / Clarify',
        recipe: [
          { name: seed.primarySpirit, amount: '35 ml' },
          { name: seed.secondarySpirit, amount: '15 ml' },
          { name: seed.cordial, amount: '20 ml' },
          { name: seed.acid, amount: '15 ml' },
          { name: '热牛奶 / Warm milk', amount: '45 ml' },
        ],
        steps: ['把酒体、酸和 Cordial 混合。', '缓慢倒入温牛奶中静置凝结。', '通过滤纸或咖啡滤杯澄清。', '冰镇后直接倒入大冰块杯，测试甜度和酸度余韵。'],
      },
      {
        id: `${slug(seed.ingredient)}-low-abv`,
        ingredient: seed.ingredient,
        name: lowAbvName,
        style: 'Low ABV / 低酒精研发',
        confidence: 'R&D starting point',
        bases: ['Vermouth / Sake / Sherry'],
        ingredients: ['Vermouth / Sake / Sherry', seed.cordial, seed.acid, seed.bubbles, seed.garnish],
        flavorTags: ['低酒精', '餐前', ...seed.tags].slice(0, 4),
        method: '兑和 / Build',
        recipe: [
          { name: 'Vermouth / Sake / Sherry', amount: '60 ml' },
          { name: seed.cordial, amount: '15 ml' },
          { name: seed.acid, amount: '7.5 ml' },
          { name: seed.bubbles, amount: '60 ml' },
          { name: seed.garnish, amount: '1 份' },
        ],
        steps: ['酒杯加冰。', '加入低酒精基底、Cordial 和少量酸。', '补气泡材料并轻轻提拉。', '适合作为餐前或第二杯的轻盈版本。'],
      },
    ],
  };
}
